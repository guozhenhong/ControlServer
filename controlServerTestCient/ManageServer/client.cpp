...
int main(int argc, char** argv)
{
    ...
    master->RegisterDeserializer(MSG_ACK_MANAGE_SERVER_CREATE_VD, new ManageServerCreateVDACKMsgDeserializer());
    
    master->RegisterDeserializer(MSG_GET_VD_LOCATE_NODE, new GetVDLocateNodeInfoMsgDeserializer());

    master->RegisterDeserializer(MSG_ACK_MANAGER_SERVER_BATCH_CREATE_VD, new ManageServerCreateBatchVDACKMsgDeserializer());
    master->RegisterSerializer(MSG_MANAGER_SERVER_BATCH_CREATE_VD, new ManageServerCreateBatchVDMsgSerializer());

    master->RegisterSerializer(MSG_MANAGE_SERVER_DELETE_VD, new ManageServerDeleteVDMsgSerializer());
    master->RegisterDeserializer(MSG_ACK_MANAGE_SERVER_DELETE_VD, new ManageServerDeleteVDACKMsgDeserializer());

    master->RegisterDeserializer(MSG_ACK_MANAGER_SERVER_BATCH_DELETE_VD, new ManageServerDeleteBatchVDACKMsgDeserializer());
    master->RegisterSerializer(MSG_MANAGER_SERVER_BATCH_DELETE_VD, new ManageServerDeleteBatchVDMsgSerializer());
