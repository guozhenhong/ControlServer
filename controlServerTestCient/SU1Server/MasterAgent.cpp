...
void MasterAgent::readBack(InReq& req)
{
    ...
    VirtualDiskFormatMessage *pFormatMsg = dynamic_cast<VirtualDiskFormatMessage*>(pMsg);
    if(pFormatMsg)
    {
        std::cout<<"the format info is: "<<pFormatMsg->m_iEchoID<<" vd ID is : "<<pFormatMsg->m_iVDID<<"cap is: "<<pFormatMsg->m_cap<<"fs type is: "<<pFormatMsg->m_iFSType<<std::endl;
        SendMsg(new VirtualDiskFormatACKMessage(pFormatMsg->m_iEchoID, pFormatMsg->m_iVDID, 1));
        
    }
     VirtualDiskDeleteMessage *pDeleteMsg = dynamic_cast<VirtualDiskDeleteMessage*>(pMsg);
    if(pDeleteMsg)
    {
        std::cout<<"the Delte info is: "<<pDeleteMsg->m_iEchoID<<" vd ID is : "<<pDeleteMsg->m_iVDID<<std::endl;
        SendMsg(new VirtualDiskDeleteACKMessage(pDeleteMsg->m_iEchoID, pDeleteMsg->m_iVDID, 0));
...
{
    RegisterServerMessage *pMsg = new RegisterServerMessage(STORAGE_SERVER_ONE_NAME, 4512);
